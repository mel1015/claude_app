...
```bash
# 한국 주식 (KOSPI/KOSDAQ)
curl -X POST "http://localhost:8080/api/v1/system/refresh-cache?type=KR"

# 미국 주식 (NYSE/NASDAQ)
curl -X POST "http://localhost:8080/api/v1/system/refresh-cache?type=US"
...
```json
{
  ...
  "conditions": [
    { "id": "c1", "field": "rsi14", "operator": "<", "value": 30 },
    {
      "id": "g1", "logic": "OR",
      "conditions": [
        { "id": "c2", "field": "close_price", "operator": ">", "value": 50000 },
        { "id": "c3", "field": "volume", "operator": ">", "value": 1000000 }
      ]
    }
  ]
}
...
지원 필드: `close_price`, `open_price`, `high_price`, `low_price`, `volume`, `change_rate`, `ma5`, `ma20`, `ma60`, `rsi14`, `macd`, `macd_signal`, `macd_hist`
...
# 종목 수 확인
db.stock_daily_cache.countDocuments({market: "KOSPI"})
